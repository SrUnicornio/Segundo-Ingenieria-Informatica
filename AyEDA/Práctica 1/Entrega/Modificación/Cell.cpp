...
int Cell::NextState(const Lattice &lattice, const Position &position)
{
  ...
  if (border == "open" || border == "o")
  {
    ...
    {
      left_ = v.value();
      right_ = lattice.GetCell(position + 2).GetState();
    }
    else if (position == size - 1)
    {
      left_ = lattice.GetCell(position - 2).GetState();
      right_ = v.value();
    }
    else
    {
  ...
  else if (border == "periodic" || border == "p")
  {
    if (position == 0)
    {
      left_ = lattice.GetCell(size - 2).GetState(); // Modificacion
    ...
    {
      left_ = lattice.GetCell(position - 2).GetState();
      right_ = lattice.GetCell(2).GetState();
    }
    else
    {
      left_ = lattice.GetCell(position - 2).GetState();
