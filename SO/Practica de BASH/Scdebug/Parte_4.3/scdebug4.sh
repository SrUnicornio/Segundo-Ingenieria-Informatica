    ...
    -gc | -ge)
      ...
      for PID in $(pgrep -u $USER '^traced_'); do
        ...
        while read -r line; do
          ...
          summary+=("PID: $PID \tName: $name \tMax_Time: $max_time \tTotal_Time: $total_time \tCalls: $calls \tErrors: $errs")
        done < <(tail -n +2 strace_summary.txt)

        if [[ "$BOOL_INV" == true ]]; then
          for (( i=${#summary[@]}-1; i>=0; i-- )); do
            echo "${summary[$i]}"
          done
        else
...
main()
{
  ...
  for i in "${!PROGRAMS[@]}"; do
    ...
    elif  [ command -v "$PROGS" > /dev/null 2>&1 ]; then
