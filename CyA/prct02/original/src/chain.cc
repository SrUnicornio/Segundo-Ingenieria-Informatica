...
Language Chain::Prefix() const {
  Language prefixes;
  prefixes.Insert(Chain(kVoidChainSymbol, alphabet_));
  if (chain_ != kVoidChainSymbol) {
    for (size_t i = 1; i <= chain_.size(); ++i) {
      std::string pre = chain_.substr(0, i);
      prefixes.Insert(Chain(pre, alphabet_));
    }
  }
  return prefixes;
}
...
Language Chain::Suffix() const {
  Language suffixes;
  suffixes.Insert(Chain(kVoidChainSymbol, alphabet_));
  if (chain_ != kVoidChainSymbol) {
    for (size_t i = 0; i < chain_.size(); ++i) {
      std::string suf = chain_.substr(i);
      suffixes.Insert(Chain(suf, alphabet_));
    }
  }
  return suffixes;
}
